...
echo.
set /p choice3=Select an option:
echo.
echo -------------------------------------------------------------
echo.
...
echo    4. Open license
echo    5. See GitHub of the launcher
...
if !choiceX! == 6 (
    ...
    if "!BATCH_VERSION!" == "!LAUNCHER_VERSION!" (mode con: cols=61 lines=4
        cls
        echo.
        echo          The launcher is already in its latest version!           
        echo.
        pause > nul
        goto OpE
...
echo -------------------------------------------------------------
echo.
echo    Before continuing, you should know that the updates
echo    they may take approximately 24 hours to upload.
echo.
echo    If OptiJuegos has just released an update,
echo    most likely you will have to wait to be able to
echo    update version using this launcher.
echo.
...
echo    doing this can cause problems.
echo.
echo    Once you understand that,
...
if !exists! == 0 (mode con: cols=61 lines=4
    ...
    echo      The version must be installed to be able to update
    pause > nul
    goto menu
) else (cd !OPTID!\..
    mode con: cols=61 lines=4
    ...
    cls
    echo.
    echo                   OptiCraft has been updated.
